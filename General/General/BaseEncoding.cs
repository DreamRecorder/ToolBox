...
namespace DreamRecorder . ToolBox . General
{
	...
	public static class BaseEncoding
	{
		...
		private static readonly Dictionary <char , byte> _base16ReverseAlphabet =
			GetAlphabet ( _BASE16_ALPHABET , true ) ;

		/// <summary>
		///     Decodes a Base16 string as described in
		///     <see cref="!:http://tools.ietf.org/html/rfc4648">RFC 4648</see>
		///     .
		/// </summary>
		...
		public static string ToBase16String ( this byte [ ] inArray )
			=> inArray . ToBase16String ( 0 , inArray . Length ) ;
		...
		private static readonly Dictionary <char , byte> _base32ReverseAlphabet =
			GetAlphabet ( _BASE32_ALPHABET , true ) ;

		/// <summary>
		///     Decodes a Base32 string as described in
		///     <see cref="!:http://tools.ietf.org/html/rfc4648">RFC 4648</see>
		///     .
		/// </summary>
		...
		public static string ToBase32String ( this byte [ ] inArray )
			=> inArray . ToBase32String ( 0 , inArray . Length ) ;
		...
		public static string ToBase32HexString ( this byte [ ] inArray )
			=> inArray . ToBase32HexString ( 0 , inArray . Length ) ;
		...
		private static string ToBase32String ( this byte [ ] inArray , int offset , int length , char [ ] alphabet )
		{
			...
			while ( inPos < inSafeEnd )
			{
				...
				outData [ outPos++ ] =
					alphabet [ ( ( inArray [ inPos ] & 0x07 ) << 2 ) | ( ( inArray [ ++inPos ] & 0xc0 ) >> 6 ) ] ;
				outData [ outPos++ ] = alphabet [ ( ( inArray [ inPos ] & 0x3e ) >> 1 ) ] ;
				outData [ outPos++ ] =
					alphabet [ ( ( inArray [ inPos ] & 0x01 ) << 4 ) | ( ( inArray [ ++inPos ] & 0xf0 ) >> 4 ) ] ;
				outData [ outPos++ ] =
					alphabet [ ( ( inArray [ inPos ] & 0x0f ) << 1 ) | ( ( inArray [ ++inPos ] & 0x80 ) >> 7 ) ] ;
				outData [ outPos++ ] = alphabet [ ( ( inArray [ inPos ] & 0x7c ) >> 2 ) ] ;
				outData [ outPos++ ] =
					alphabet [ ( ( inArray [ inPos ] & 0x03 ) << 3 ) | ( ( inArray [ ++inPos ] & 0xe0 ) >> 5 ) ] ;
		...
		private static readonly Dictionary <char , byte> _base64ReverseAlphabet =
			GetAlphabet ( _BASE64_ALPHABET , false ) ;

		/// <summary>
		///     Decodes a Base64 string as described in
		///     <see cref="!:http://tools.ietf.org/html/rfc4648">RFC 4648</see>
		///     .
		/// </summary>
		...
		private const string _BASE64_URL_ALPHABET =
		...
		public static string ToBase64UrlString ( this byte [ ] inArray )
			=> inArray . ToBase64UrlString ( 0 , inArray . Length ) ;
			...
			if ( remain > 0 )
			{
				...
				switch ( remain )
				{
					...
					case 2 :
						res [ outPos++ ] = ( byte )( ( buffer [ 0 ] << 2 ) | ( ( buffer [ 1 ] >> 4 ) & 0x03 ) ) ;
						res [ outPos ] =
							( byte )( ( ( buffer [ 1 ] << 4 ) & 0xf0 ) | ( ( buffer [ 2 ] >> 2 ) & 0x0f ) ) ;
						break ;
				}
			}
		...
		private static string ToBase64String ( this byte [ ] inArray , int offset , int length , char [ ] alphabet )
		{
			...
			while ( inPos < inSafeEnd )
			{
				...
				outData [ outPos++ ] =
					alphabet [ ( ( inArray [ inPos ] & 0x03 ) << 4 ) | ( ( inArray [ ++inPos ] & 0xf0 ) >> 4 ) ] ;
				outData [ outPos++ ] =
					alphabet [ ( ( inArray [ inPos ] & 0x0f ) << 2 ) | ( ( inArray [ ++inPos ] & 0xc0 ) >> 6 ) ] ;
