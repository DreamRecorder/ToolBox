	...
	: IDnsSecResolver , IInternalDnsSecResolver <DnsSecValidatorContext>
{
	...
	public Task <DnsSecResult <T>> ResolveSecureAsync <T> (
		...
		RecordClass       recordClass = RecordClass.INet ,
		CancellationToken token       = default ( CancellationToken ) ) where T : DnsRecordBase
		=> ResolveSecureInternalAsync <T> ( name , recordType , recordClass , new DnsSecValidatorContext ( ) , token );
		...
		CancellationToken      token ) where T : DnsRecordBase
	{
		...
		if ( cName != null )
		{
			DnsSecValidationResult cNameValidationResult = await _validator.ValidateAsync (
															name ,
															RecordType.CName ,
															recordClass ,
															msg ,
			...
			if ( records.Count > 0 )
			{
				DnsSecValidationResult recordsValidationResult =
					await _validator.ValidateAsync (
													...
													state ,
													token );
				if ( recordsValidationResult is DnsSecValidationResult.Bogus or DnsSecValidationResult.Indeterminate )
				{
