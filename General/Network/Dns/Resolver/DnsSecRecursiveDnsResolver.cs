...
namespace DreamRecorder . ToolBox . Network . Dns . Resolver
{
	...
	///     </para>
	/// </summary>
	public class DnsSecRecursiveDnsResolver
		: IDnsSecResolver , IInternalDnsSecResolver <DnsSecRecursiveDnsResolver . State>
	{
		...
		Task <DnsMessage> IInternalDnsSecResolver <State> . ResolveMessageAsync (
			DomainName        name ,
		...
		Task <DnsSecResult <TRecord>> IInternalDnsSecResolver <State> . ResolveSecureAsync <TRecord> (
			DomainName        name ,
			RecordType        recordType ,
			RecordClass       recordClass ,
			State             state ,
		...
		private async Task <DnsSecResult <T>> ResolveAsyncInternal <T> (
			DomainName        name ,
			RecordType        recordType ,
			RecordClass       recordClass ,
			State             state ,
			CancellationToken token ) where T : DnsRecordBase
		{
			...
			if ( _cache . TryGetRecords ( name , RecordType . CName , recordClass , out DnsCacheRecordList <CNameRecord> cachedCNames ) )
			{
				...
				DnsSecResult <T> cNameResult =
					await ResolveAsyncInternal <T> ( cNameCanonicalName , recordType , recordClass , state , token ) ;
				return new DnsSecResult <T> (
											cNameResult . ReturnCode ,
											cNameResult . Records ,
			...
			if ( cNameRecords . Count > 0 )
			{
				...
				{
					DnsSecValidationResult matchingValidationResult =
						await _validator . ValidateAsync (
														canonicalName ,
														recordType ,
														recordClass ,
					...
					DnsSecValidationResult validationResult =
						cNameValidationResult == matchingValidationResult
							? cNameValidationResult
							: DnsSecValidationResult . Unsigned ;
					_cache . Add (
								canonicalName ,
					...
					return new DnsSecResult <T> (
												msg . ReturnCode ,
												matchingAdditionalRecords . OfType <T> ( ) . ToList ( ) ,
												validationResult ) ;
				}
				...
				DnsSecResult <T> cNameResults =
					await ResolveAsyncInternal <T> ( canonicalName , recordType , recordClass , state , token ) ;
				return new DnsSecResult <T> (
											cNameResults . ReturnCode ,
			...
			CancellationToken token )
		{
			...
			DnsSecResult <AaaaRecord> aaaaRecords =
