...
public class DnsSecRecursiveDnsResolver : IDnsSecResolver , IInternalDnsSecResolver <DnsSecRecursiveDnsResolver.State>
{
			...
			if ( matchingAdditionalRecords.Count > 0 )
			{
				DnsSecValidationResult matchingValidationResult =
					await _validator.ValidateAsync (
													...
													state ,
													token );
				if ( matchingValidationResult is DnsSecValidationResult.Bogus or DnsSecValidationResult.Indeterminate )
				{
		...
		CancellationToken token )
	{
		...
		result.AddRange (
						 aaaaRecords.Records.Select ( x => new Tuple <IPAddress , int> ( x.Address , x.TimeToLive ) ) );
