...
public abstract class DnsClientBase
{
	...
	private List <DnsClientEndpointInfo> GetEndpointInfos ( )
	{
		...
		{
			endpointInfos = _servers.Where ( x => IsIPv6Enabled || ( x.AddressFamily == AddressFamily.InterNetwork ) ).
									 Select (
	...
	private void PrepareMessage <TMessage> (
		...
		out byte [ ]                tsigOriginalMac ) where TMessage : DnsMessageBase , new ( )
	{
	...
	private byte [ ] QueryByTcp (
		IPAddress         nameServer ,
		byte [ ]          messageData ,
		int               messageLength ,
		ref TcpClient     tcpClient ,
		ref NetworkStream tcpStream ,
		...
		try
		{
			if ( tcpClient == null )
			{
				tcpClient = new TcpClient ( nameServer.AddressFamily )
							{
								ReceiveTimeout = QueryTimeout , SendTimeout = QueryTimeout ,
							};
	...
	protected TMessage SendMessage <TMessage> ( TMessage message ) where TMessage : DnsMessageBase , new ( )
	{
		...
		bool sendByTcp =
			( ( messageLength > MaximumQueryMessageSize ) || message.IsTcpUsingRequested || ! IsUdpEnabled );
	...
	private async Task SendMessageParallelAsync <TMessage> (
		...
		CancellationToken             token ) where TMessage : DnsMessageBase , new ( )
	{
		using UdpClient udpClient      = new UdpClient ( new IPEndPoint ( endpointInfo.LocalAddress , 0 ) );
		IPEndPoint      serverEndpoint = new IPEndPoint ( endpointInfo.ServerAddress , _port );
		await udpClient.SendAsync ( messageData , messageLength , serverEndpoint );
		...
		{
			TMessage         result;
			UdpReceiveResult response = await udpClient.ReceiveAsync ( int.MaxValue , token );
