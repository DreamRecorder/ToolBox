...
using DreamRecorder . ToolBox . Network . Dns . DynamicUpdate ;
...
namespace DreamRecorder . ToolBox . Network . Dns
{
	...
	public abstract class DnsMessageBase
	{
		...
		internal static DnsMessageBase CreateByFlag (
			byte [ ]                  data ,
		...
		internal static TMessage Parse <TMessage> ( byte [ ] data ) where TMessage : DnsMessageBase , new ( )
			=> Parse <TMessage> ( data , null , null ) ;

		internal static TMessage Parse
			<TMessage> ( byte [ ] data , DnsServer . SelectTsigKey tsigKeySelector , byte [ ] originalMac )
		...
		private void ParseInternal ( byte [ ] data , DnsServer . SelectTsigKey tsigKeySelector , byte [ ] originalMac )
		{
			...
			{
				int tSigPos =
					_additionalRecords . FindIndex ( record => ( record . RecordType == RecordType . TSig ) ) ;
		...
		private ReturnCode ValidateTSig (
			byte [ ]                  resultData ,
			DnsServer . SelectTsigKey tsigKeySelector ,
			byte [ ]                  originalMac )
		{
		...
		private static void ParseSection (
			...
			List <DnsRecordBase> sectionList ,
			int                  recordCount )
		{
		...
		{
			DomainName res = ParseDomainName ( resultData , currentPosition , out int firstLabelLength ) ;
		...
		internal static int ParseInt ( byte [ ] resultData , ref int currentPosition )
		{
			...
			if ( BitConverter . IsLittleEndian )
			{
				res = ( ( resultData [ currentPosition++ ]   << 24 )
						| ( resultData [ currentPosition++ ] << 16 )
						| ( resultData [ currentPosition++ ] << 8 )
						| resultData [ currentPosition++ ] ) ;
			}
			else
			{
				res = ( resultData [ currentPosition++ ]
						| ( resultData [ currentPosition++ ] << 8 )
						| ( resultData [ currentPosition++ ] << 16 )
						| ( resultData [ currentPosition++ ] << 24 ) ) ;
			}
		...
		internal static uint ParseUInt ( byte [ ] resultData , ref int currentPosition )
		{
			...
			if ( BitConverter . IsLittleEndian )
			{
				res = ( ( ( uint )resultData [ currentPosition++ ]   << 24 )
						| ( ( uint )resultData [ currentPosition++ ] << 16 )
						| ( ( uint )resultData [ currentPosition++ ] << 8 )
						| resultData [ currentPosition++ ] ) ;
			}
			else
			{
				res = ( resultData [ currentPosition++ ]
						| ( ( uint )resultData [ currentPosition++ ] << 8 )
						| ( ( uint )resultData [ currentPosition++ ] << 16 )
						| ( ( uint )resultData [ currentPosition++ ] << 24 ) ) ;
			}
		...
		private static DomainName ParseDomainName (
			byte [ ] resultData ,
			int      currentPosition ,
		...
		internal int Encode (
			...
			byte [ ]     originalTsigMac ,
			bool         isSubSequentResponse ,
		...
		internal static void EncodeDomainName (
			byte [ ]                         messageData ,
			int                              offset ,
			ref int                          currentPosition ,
			DomainName                       name ,
			...
			bool                             useCanonical )
		{
			...
			if ( isCompressionAllowed && domainNames . TryGetValue ( name , out ushort pointer ) )
			{
		...
		internal static void EncodeByteArray (
			byte [ ] messageData ,
			ref int  currentPosition ,
			byte [ ] data ,
			int      length )
