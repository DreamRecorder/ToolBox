...
public class DnsClient : DnsClientBase
{
	...
	/// </summary>
	public new bool IsTcpEnabled { get => base.IsTcpEnabled; set => base.IsTcpEnabled = value; }

	/// <summary>
	///     Gets or sets a value indicationg whether queries can be sent using UDP.
	/// </summary>
	public new bool IsUdpEnabled { get => base.IsUdpEnabled; set => base.IsUdpEnabled = value; }
	...
	public DnsClient ( IPAddress dnsServer , int queryTimeout ) : this (
																		new List <IPAddress> { dnsServer , } ,
																		queryTimeout )
	{
	}
	...
	/// <param name="queryTimeout"> Query timeout in milliseconds </param>
	public DnsClient ( IEnumerable <IPAddress> dnsServers , int queryTimeout ) : base ( dnsServers , queryTimeout , 53 )
	{
		IsUdpEnabled = true;
		IsTcpEnabled = true;
	}
			...
			try
			{
				...
				while ( ( line = reader.ReadLine ( ) ) != null )
				{
					...
					if ( ( lineData.Length   == 2 )
						 && ( lineData [ 0 ] == "nameserver" )
						 && ( IPAddress.TryParse ( lineData [ 1 ] , out IPAddress dns ) ) )
					{
						res.Add ( dns );
					}
				}
	...
	/// <param name="token"> The token to monitor cancellation requests </param>
	/// <returns> The complete response of the dns server </returns>
	public Task <DnsMessage> SendMessageAsync (
		DnsMessage        message ,
		CancellationToken token = default ( CancellationToken ) )
	{
		if ( message == null )
	...
	/// <param name="token"> The token to monitor cancellation requests </param>
	/// <returns> The complete response of the dns server </returns>
	public Task <DnsUpdateMessage> SendUpdateAsync (
		DnsUpdateMessage  message ,
		CancellationToken token = default ( CancellationToken ) )
	{
		if ( message == null )
		{
