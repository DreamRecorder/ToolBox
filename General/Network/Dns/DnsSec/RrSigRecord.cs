...
public class RrSigRecord : DnsRecordBase
{
		...
		foreach ( T record in records.OrderBy ( x => x ) )
		{
			...
			else if ( record.Name.LabelCount > Labels )
			{
				DnsMessageBase.EncodeDomainName (
												 ...
												 ref length ,
												 DomainName.Asterisk
												 + record.Name.GetParentName ( record.Name.LabelCount - Labels ) ,
												 null ,
												 true );
			}
			else
	...
	internal bool Verify <T> ( List <T> coveredRecords , IEnumerable <DnsKeyRecord> dnsKeys ) where T : DnsRecordBase
	{
