...
internal class DnsSecValidator <TState> where TState : IDnsSecValidatorContext
{
		...
		CancellationToken  token )
	{
		...
		if ( checkWildcard )
		{
			DomainName wildcardHashName = ( DomainName.Asterisk + current ).GetNsec3HashName (
			 nsec3Parameter.HashAlgorithm ,
			 nsec3Parameter.Iterations ,
			 nsec3Parameter.Salt ,
			 zoneApex );

			NSec3Record wildcardDirectMatch = nsecRecords.FirstOrDefault ( x => x.Name.Equals ( wildcardHashName ) );
			...
			NSec3Record wildcardCoveringMatch = nsecRecords.FirstOrDefault ( x => x.IsCovering ( wildcardHashName ) );
