	...
	where TSettingCategory : Enum , IConvertible
{
	...
	public new static T Current { get => ( T )ProgramBase.Current; private set => ProgramBase.Current = value; }
		...
		int Execution ( )
		{
			...
			else
			{
				if ( WriteLicenseFile )
				{
					if ( ! File.Exists ( LicenseFilePath ) )
					{
						Logger.LogInformation ( $"You can found license of this program at \"{LicenseFilePath}\"." );
						GenerateLicenseFile ( );
					}
				}
			}
	...
	public virtual void ShowLogo ( )
	{
		...
		logo.AppendLine (
						 new AsciiArt ( typeof ( T ).Assembly.GetProgramName ( ) , width : CharacterWidth.Smush ).
							 ToString ( ) );
